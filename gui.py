...
class _SettingsVars(TypedDict):
    ...
    # TDM_CLAIM_TOGGLE_PATCH_BEGIN: gui-var-schema
    auto_claim_drops: IntVar
    # TDM_CLAIM_TOGGLE_PATCH_END: gui-var-schema
...
class SettingsPanel:
    ...
    def __init__(self, manager: GUIManager, master: ttk.Widget):
        ...
        self._vars: _SettingsVars = {
            ...
            # TDM_CLAIM_TOGGLE_PATCH_BEGIN: gui-var
            "auto_claim_drops": IntVar(
                master, int(self._settings.auto_claim_drops)
            ),
            # TDM_CLAIM_TOGGLE_PATCH_END: gui-var
        }
        ...
        # TDM_CLAIM_TOGGLE_PATCH_BEGIN: gui-toggle
        ttk.Label(
            checkboxes_frame, text=_("gui", "settings", "general", "auto_claim_drops")
        ).grid(column=0, row=(irow := irow + 1), sticky="e")
        ttk.Checkbutton(
            checkboxes_frame,
            variable=self._vars["auto_claim_drops"],
            command=self.update_auto_claim_drops,
        ).grid(column=1, row=irow, sticky="w")
        ttk.Label(
            checkboxes_frame,
            text=_("gui", "settings", "general", "auto_claim_drops_help"),
            wraplength=430,
            justify="left",
        ).grid(column=0, row=(irow := irow + 1), columnspan=2, sticky="w")
        # TDM_CLAIM_TOGGLE_PATCH_END: gui-toggle
        ttk.Label(
    ...
    # TDM_CLAIM_TOGGLE_PATCH_BEGIN: gui-toggle-callback
    def update_auto_claim_drops(self) -> None:
        enabled = bool(self._vars["auto_claim_drops"].get())
        self._settings.auto_claim_drops = enabled
        self._settings.save()
        if enabled:
            self._manager._twitch.clear_claim_progression_blocks()
            self._manager._twitch.change_state(State.INVENTORY_FETCH)
    # TDM_CLAIM_TOGGLE_PATCH_END: gui-toggle-callback
